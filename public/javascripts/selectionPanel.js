...
q.defer(d3.csv,csvData.distance.directory)
  .defer(d3.csv,csvData.auto.directory)
    .defer(d3.csv,csvData.transit.directory)
      ...
      .await(startApp);
function startApp(error,distance,auto,transit,walk){
    ...
    "esri/Color", "dojo/dom-style", "dojo/domReady!"
      ...
      function fillPersonalInfoPanel(){
          ...
          for(var i=0;i<householdInfo.totalMembers;i++){
            ...
            $('#'+divId).append('<h4 style="text-align:left; margin-left:20px;margin-top:0px">'+numberList[i]+' person:<h4>')
            ...
            $('#'+divId).append('<input style="width:150px" type="text" id="'+divAgeId+'">');
            ...
            $('#'+divOccupId).append('<option value="student">Student at Edmonton</option>');
          }
      ...
      function fillPlacePanel(){
         for(var i=0;i<personList.length;i++){
            if(personList[i].occupation!=='other'){
                ...
                $('#'+divId).append('<label for="'+divTravelMethod+'">Travel Method:&nbsp</label>');
                $('#'+divId).append('<select style="width:150px" autocomplete="off" id="'+divTravelMethod+'"></select>');
                $('#'+divTravelMethod).append('<option value="transit" selected>Public Transit</option>');
          ...
          for(var j=0;j<personList.length;j++){
            if(personList[j].address || personList[j].address!==null){
                ...
                if(dataArray){
                  ...
                  timeResult[allZones[i]]+=100000000000000;
                }
